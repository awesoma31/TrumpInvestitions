...
3. [Архитектура системы](#архитектура-системы)
   ...
   2. [Архитектура клиентского приложения](#архитектура-клиентского-приложения)
   3. [Архитектура базы данных](#архитектура-базы-данных)
   4. [Архитектура потоков данных](#архитектура-потоков-данных)
   5. [HTTP API сервисов](#http-api-сервисов)
4. [Описание реализации](#описание-реализации)
   ...
   5. [Android-клиент](#android-клиент)
   6. [Load Service](#load-service)
   7. [Генерация и загрузка рыночных данных](#генерация-и-загрузка-рыночных-данных)
   8. [Сквозная идея реализации](#сквозная-идея-реализации)
...
1. Доступ к рыночным данным и аналитике
   1. Хранение и генерация котировок
      1. Система должна загружать исторические котировки.
      2. Система должна генерировать котировки в реальном времени.
   2. Получение котировок
      1. Система должна отображать список котировок для каждой акции на клиентском UI.
      2. Система должна отрисовывать историю цены отдельной акции на клиентском UI в виде графика зависимости цены от времени.
   3. Система должна предоставлять возможность просмотра всех акций списком.
   4. Система должна предоставлять возможность просмотра всех Bid/Ask в виде списка для каждой акции.
2. Совершение торговых операций
   1. Система должна предоставлять возможность купить или продать акцию, а также пакет акций, по заявке.
   2. Система должна учитывать доступность баланса при покупке акций.
   3. Система должна учитывать число акций в портфеле при продаже акций.
3. Управление инвестиционным портфелем
   1. Просмотр состояния портфеля
      1. Система должна предоставлять возможность просмотра истории сделок.
      2. Система должна предоставлять возможность расчёта доходности PnL.
      3. Система должна предоставлять возможность просмотра списка акций пользователя.
      4. Система должна предоставлять возможность просмотра истории заявок пользователя.
4. Регистрация и вход в систему
   1. Процесс аутентификации
      1. Система должна предоставлять возможность регистрации нового пользователя с логином и паролем.
      2. Система должна предоставлять возможность входа в систему по логину и паролю.
      3. Система должна обеспечивать выдачу JWT-токена при успешной авторизации.
      4. Система должна обеспечивать проверку токена при каждом запросе.
      5. Система должна предоставлять возможность выхода из системы и отзыва токена (инвалидации).
   2. Неавторизованный пользователь может только просматривать график цен, список акций и заявок на них.
5. Обработка запросов через API Gateway
   1. Централизованная маршрутизация запросов
      1. Система должна обеспечивать приём всех HTTP-запросов от клиента.
      2. Система должна обеспечивать получение ответов от микросервисов.
      3. Система должна логировать все операции и предоставлять мониторинг доступности.
6. Логирование и мониторинг
   1. Работа с логами
      1. Система должна обеспечивать приём логов от всех микросервисов.
7. Нагрузочное тестирование
   1. Проведение нагрузочных тестов
      1. Система должна обеспечивать генерацию синтетических пользователей.
      2. Система должна обеспечивать запуск типовых сценариев с высокой частотой.
      3. Система должна отображать время отклика микросервисов.l
...
- `android-client` реализует пользовательский интерфейс, управление сессией и отображение рыночных данных, портфеля и торговых операций;
...
Пользовательский фронт проекта реализован в виде нативного Android-приложения `front-ktl`, написанного на Kotlin с использованием Jetpack Compose. Клиент встроен в общую архитектуру как самостоятельный слой представления: он не содержит собственной бизнес-логики торговли или расчёта портфеля, а обращается к backend API через `auth-gateway`, получая уже согласованные данные из доменных сервисов.

На уровне структуры приложение разделено на несколько слоёв:

- слой UI на Compose отвечает за экраны авторизации, список инструментов, карточку акции, экран портфеля и профиль пользователя;
- навигационный слой на `Navigation Compose` управляет переходами между auth-flow и основным приложением, а также между вкладками рынка, портфеля и профиля;
- слой `ViewModel` хранит состояние экранов, управляет загрузкой данных и изолирует Compose-компоненты от сетевого взаимодействия;
- слой `Repository` инкапсулирует вызовы backend API и преобразует DTO-модели в внутренние модели клиента;
- сетевой слой `AppNetwork` построен на Retrofit и OkHttp, добавляет авторизационные заголовки, обновление access token и OpenTelemetry-инструментацию.

Отдельное место занимает управление пользовательской сессией. Access token и refresh token сохраняются локально через `DataStore`, а состояние авторизации предоставляется в виде реактивного `Flow`. За счёт этого корневой Compose-граф может автоматически переключаться между экранами входа и основным пользовательским интерфейсом без ручной синхронизации состояния между экранами.

С точки зрения обмена данными клиент использует pull-модель. Экран рынка регулярно обновляет котировки, а экран портфеля периодически перечитывает агрегированное состояние портфеля и историю ордеров. Такой подход соответствует текущему backend-контракту проекта, где основной доступ к данным организован через HTTP snapshot-запросы, а не через постоянные WebSocket-подключения.
...
Android-клиент реализован в модуле `front-ktl` на Kotlin и Jetpack Compose. Входной точкой приложения является `MainActivity`, где инициализируется Compose-дерево, а в классе `TrumpApp` создаются долгоживущие зависимости: `TokenManager`, `SettingsManager`, экземпляр `AppNetwork` и OpenTelemetry SDK для отправки клиентских трейсов. Благодаря этому сетевой слой, хранилище токенов и параметры окружения доступны во всех частях приложения через единый `Application`-контекст.

Навигация построена в два уровня. Сначала приложение реактивно определяет, авторизован ли пользователь, через `isLoggedInFlow()`. Если access token отсутствует, запускается отдельный auth-flow с экранами входа и регистрации. После успешной авторизации пользователь попадает в основное приложение, где `NavigationSuiteScaffold` формирует нижнюю навигацию между разделами рынка, портфеля и профиля, а детальный экран акции открывается как отдельный маршрут с параметром `symbol`.

Состояние экранов вынесено во `ViewModel`, что позволяет отделить UI от сетевых вызовов. `AuthViewModel` отвечает за вход, регистрацию и logout, `StocksViewModel` поддерживает список инструментов, `StockDetailViewModel` загружает свечи, текущую котировку и упрощённый стакан, а также отправляет рыночные заявки на покупку и продажу. `PortfolioViewModel` и `ProfileViewModel` используют данные портфельного API для отображения текущего баланса, позиций и пользовательских сведений.

Взаимодействие с backend инкапсулировано в `AppNetwork`, `AuthRepository`, `MarketRepository` и `PortfolioRepository`. Retrofit-клиенты разделены на публичный auth-контур и авторизованный API-контур. Для защищённых запросов OkHttp добавляет `AuthInterceptor` с access token, а `TokenRefreshAuthenticator` автоматически выполняет обновление токена при получении `401 Unauthorized`. Это позволяет клиенту продолжать пользовательскую сессию без повторного ручного входа при истечении короткоживущего access token.

Рыночные данные в текущей версии клиента обновляются периодическим polling-механизмом. `MarketRepository` перечитывает список котировок примерно раз в 5 секунд, а `PortfolioRepository` запрашивает состояние портфеля и историю ордеров раз в 10 секунд. Для экрана отдельной акции дополнительно поддерживаются временные масштабы графика (`1Ч`, `4Ч`, `1Д`, `1Н`, `1М`), на основе которых клиент формирует запросы исторических свечей в `market-data-service`. Такая схема достаточно проста в реализации и хорошо согласуется с учебным характером проекта, где приоритетом является воспроизводимость пользовательских сценариев, а не минимальная задержка обновления интерфейса.
